...
def test_transfer_reobserves_when_named_source_maps_to_anonymous_candidate(
    monkeypatch,
) -> None:
    import omniflow.runtime.core as core
    import omniflow.runtime.execution as execution
    ...
    ready = Observation(package_name="com.sankuai.meituan", xml="<merchant/>")
    after = Observation(package_name="com.sankuai.meituan", xml="<search/>")
    transfer_observations: list[Observation] = []
    ...
    async def transfer(action, observation, source_state):
        ...
        if observation == loading:
            return TransferResult(
                Action("click", {"x": 600, "y": 157}),
                reason="mutual_graph_matcher_no_null_v3",
                detail={
    ...
    assert result.success is True
    assert host.actions == [Action("click", {"x": 350, "y": 180})]
    assert transfer_observations == [loading, ready]
    assert result.detail["observation_retry"] == {
        "attempts": 1,
        "initial_reason": "mutual_graph_matcher_no_null_v3",
    }


def test_transfer_executes_top_candidate_after_semantic_reobserve_budget(
    monkeypatch,
) -> None:
    import omniflow.runtime.core as core
    import omniflow.runtime.execution as execution
    ...
    async def transfer(action, observation, source_state):
        return TransferResult(
            ...
            detail={
                "mapping_mode": "mutual_graph_matcher_no_null_v3",
                "source": {"content_desc": "搜索"},
                "candidates": [{"class": "android.view.ViewGroup", "score": 0.1}],
                "score": 0.999,
            },
        )
    ...
    assert result.success is True
    assert host.actions == [Action("click", {"x": 600, "y": 157})]
    assert host.observe_calls == 3
    assert result.detail["observation_retry"] == {
        "attempts": 2,
        "initial_reason": "mutual_graph_matcher_no_null_v3",
