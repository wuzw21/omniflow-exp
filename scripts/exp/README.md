...
内置 Harness。严格 Memory ON/OFF 配对保留同一个显式 Store，使用
`--function-memory on|off`；off 只关闭内置 Function 暴露/执行，并独立归档为
`memory_ablation`。原不传 Memory 的 baseline 改变模型超时和 Host 等待配置，
不得混入这一单因素配对。
