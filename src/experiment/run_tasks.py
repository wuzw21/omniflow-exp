...
def prepare_function_asset(
    *,
    ...
    """Convert one successful RunLog into the temporary OmniFlow memory."""

    del run_log, deadline
    report = compile_function_v2(
        source_path,
        attempt_root / "omniflow_memory",
        enhance=True,
        model=args.formal_model,
    )
    store_path = Path(str(report["store_path"])).resolve()
...
def prepare_mobilegpt_memory(
    *,
    ...
    """Convert the same source RunLog with MobileGPT's memory adapter."""
    ...
    if explicit:
        ...
        return root, {"status": "provided", "memory_root": str(root)}

    _, source_path, _ = _canonical_source(args.memory_index, args.task)
    conversion = convert_runlog_to_mobilegpt_bundle(
        source_run_log=source_path,
        mobilegpt_root=args.mobilegpt_root,
        output_root=attempt_root / "mobilegpt_memory",
        model=args.formal_model,
        embedding_model=(
    ...
    root = Path(str(conversion["memory_root"])).resolve()
    return root, {
        "status": "created",
        "memory_root": str(root),
        "model_calls": int(
            dict(conversion.get("sealed") or {}).get("model_calls") or 0
        ),
    }
...
def prepare_appagent_memory(
    *,
    ...
    """Run AppAgent's one memory-conversion command."""
    ...
    root = attempt_root / "appagent_memory"
    command = [
        str(args.python_bin),
        "-m",
    ...
    completed = subprocess.run(
        ...
        capture_output=True,
        text=True,
        timeout=max(1.0, deadline.remaining(TASK_DEADLINE_SEC)),
        check=False,
    )
    if completed.returncode != 0:
        message = (completed.stderr or completed.stdout).strip()
        raise RuntimeError(
            f"appagent_memory_prep_failed:{completed.returncode}:{message}"
        )
    return root, {"status": "created", "memory_root": str(root)}
...
def prepare_autodroid_memory(
    *,
    ...
    """Pass the configured official AutoDroid memory to its runner."""

    del attempt_root, deadline
    root = Path(str(args.autodroid_memory_root)).expanduser().resolve()
    return root, {"status": "provided", "memory_root": str(root)}
...
def run_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    """Prepare each method's memory once, then run the requested task."""
    ...
    with tempfile.TemporaryDirectory(
        ...
        attempt_root = Path(temporary_root)
        ...
        blocked: dict[str, tuple[str, str, str]] = {}
        ...
        preparations = (
            ...
            (
                "mobilegpt",
                "mobilegpt_memory",
                lambda: prepare_mobilegpt_memory(
                    args=args,
                    attempt_root=attempt_root,
                    deadline=deadline,
                ),
            ...
            (
                "appagent",
                "appagent_memory",
                lambda: prepare_appagent_memory(
                    args=args,
                    attempt_root=attempt_root,
                    deadline=deadline,
                    source_run_log=source_path,
            ...
            (
                "autodroid",
                "autodroid_memory",
                lambda: prepare_autodroid_memory(
                    args=args,
                    attempt_root=attempt_root,
                    deadline=deadline,
                ),
        ...
        for method, phase_name, prepare in preparations:
            if method not in methods:
                continue
            try:
                prepared[method], phases[phase_name] = prepare()
            except Exception as error:
                phases[phase_name] = {
                    "status": "failed",
                    "error": f"{type(error).__name__}: {error}",
                }
                blocked[method] = ("failed", "memory", str(error))

        function_store = prepared.get("omniflow")
        store_path = (
            Path(str(function_store["store_path"]))
        ...
        phases["targets"] = {
            ...
            "workers": run_target_workers(
                ...
                store_path=store_path,
                mobilegpt_memory=prepared.get("mobilegpt"),
                appagent_memory=prepared.get("appagent"),
                autodroid_memory=prepared.get("autodroid"),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--task", "--e2e-task", dest="task", default=DEFAULT_TASK)
    parser.add_argument(
        "--method",
        "--e2e-method",
        dest="e2e_method",
    ...
    parser.add_argument(
        ...
        choices=("oob",),
        default="oob",
        help="AndroidWorld physical I/O backend (OOB is mandatory).",
    )
