...
GUI_AGENT_RULES = (
    ...
    "History is factual: completed actions and Function steps already happened, so preserve their effects and do not repeat them. Function completion is progress, not proof that the whole Task is complete. If a completed Function's description covers the goal and the current UI is consistent with it, return `finished` before taking another action.",
    "Call `finished` only when the current UI proves the full goal is complete; for list or information tasks, do not treat a partial viewport or a few matching rows as the complete answer. Pair each item with its visible condition field and inspect the whole scrollable list; use a filter only when the UI clearly provides a global filter, not an item-level field.",
    "A completed Function does not answer the Task unless its description explicitly provides the requested result. For a scrollable list, read title/condition pairs and swipe through unseen rows; do not click read-only rows or per-item condition fields to filter.",
    ...
    "For value-setting or state-setting goals, do not return `finished` after only navigation, scrolling, or an action reported as successful. First verify the requested value or state in the fresh accessibility observation or screenshot.",
