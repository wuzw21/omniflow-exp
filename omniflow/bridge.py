...
class JsonLineBridge:
    ...
    def _run(
        ...
        def model_turn_transport(envelope: dict[str, Any]) -> dict[str, Any]:
            response = self.host_call(request_id, "model_turn", envelope)
            if not isinstance(response, dict):
                raise TypeError("model_turn_response_invalid")
            return response

        def publish_planner_metadata(metadata: dict[str, Any]) -> None:
            host.current_action_metadata = dict(metadata)
